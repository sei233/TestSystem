...
public interface Test2Repository extends JpaRepository<Test2,Long>,JpaSpecificationExecutor<Test2> {
    ...
    public Test2 findByQuestion(String question);
    //要想用sql语句直接写复杂查询，按照这个这么写就行了，?1是参数占位符,注意这里就没有按照规则findBy出来
    @Query(value = "SELECT * FROM testjudge WHERE user_name=?1 AND user_id=?2",nativeQuery = true)
    public Test2 findUserByUserNameAndUserId(String userName, Long userId);

    @Query(value = "SELECT * FROM testjudge limit ?1,?2",nativeQuery = true)
    List<Test2> selectTest(int index, int size);

    @Query(value = "SELECT COUNT(question) from testjudge",nativeQuery = true)
    int getCount();
}
